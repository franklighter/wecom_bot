...
AI WeWork Robot是一个基于Python的企业微信机器人，能够接入GPT、DashScope等大型语言模型。默认配置为DashScope (qwen-plus)，它可以帮助企业自动化处理信息，提供智能回复等功能。
...
dashscope_api_key = 从阿里云DashScope控制台获取的API Key (格式: sk-xxx)
```
保存，启动程序
```bash
python3 main.py
